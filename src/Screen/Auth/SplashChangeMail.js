...
/**
 * Reusable ChangeMailSplash component.
 * 
 * @component
 * @param {Object} props - The component props.
 * @returns {JSX.Element}
 */
const ChangeMailSplash = (props) => {
    ...
    if (status == '' || AuthReducer.status != status) {
        switch (AuthReducer.status) {
            ...
            case 'Auth/changeemailSuccess':
                status = AuthReducer.status;
                props.navigation.navigate("VerifyOTPEmail", { NewEmail: { "email": email, verifyotp: AuthReducer?.changeemailResponse?.email_otp,returnDat:props?.route?.params?.wholeData } });
                break;
        ...
        <>
            ...
            >
                <SafeAreaView style={{ flex: 1, backgroundColor: Colorpath.Pagebg }}>
                    ...
                    <View>
                        <Header
                            onPress={() => props.navigation.navigate("VerifyOTPEmail",{NewEmail:props?.route?.params?.wholeData})}
                    ...
                    <View style={{ flexDirection: "row", justifyContent: "center", marginTop: normalize(20) }}>
                        <TouchableOpacity onPress={() => { props.navigation.navigate("VerifyOTPEmail",{NewEmail:props?.route?.params?.wholeData || AuthReducer?.verifyResponse?.email}); }}>
