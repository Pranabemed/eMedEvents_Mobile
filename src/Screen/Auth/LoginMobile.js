...
const LoginMobile = (props) => {
    ...
    console.log(DashboardReducer?.mainprofileResponse, "mainprofile---------", props?.route?.params, WebcastReducer)
    ...
    useEffect(() => {
        if (props?.route?.params?.validPh?.cellno && !AuthReducer?.resendmobileotpResponse?.phone_otp) {
            setTimeout(() => {
                setLogintext("login");
            }, 2000);
        }
    }, [props?.route?.params?.validPh?.cellno, AuthReducer?.resendmobileotpResponse?.phone_otp])
    ...
    // Step 1: Check if resend button or OTP input should be enabled
    const isEnabledMobile = countdown > 0;

    // Step 2: Dynamically detect any available phone number from multiple possible props
    const phoneDetect =
    ...
    // Step 3: Identify a possible country code (fallback logic)
    const phoneCode =
    ...
    // Step 4: Construct base phone (add code only if missing)
    let phoneFinal = phoneDetect?.startsWith('+')
        ? phoneDetect
        : `${phoneCode}${phoneDetect || ''}`;

    // Step 5: Normalize: ensure it starts with '+'
    if (phoneFinal && !phoneFinal.startsWith('+')) {
        phoneFinal = '+' + phoneFinal;
    }

    // Step 6: Remove duplicate country codes dynamically
    phoneFinal = phoneFinal.replace(/^(\+\d+)\1+/, '$1');

    // Step 7: Clean up any stray spaces or hyphens (optional, but good)
    phoneFinal = phoneFinal.replace(/[ -]+/g, '');

    // ✅ Final sanitized output
    console.log('Final Mobile:', phoneFinal);

    console.log(phoneFinal, phoneDetect, "phonedetect=========", AuthReducer?.verifyResponse?.phone, props?.route?.params?.Newphone?.allNo)
    useEffect(() => {
                            ...
                            <View style={{ flexDirection: "row", gap: 5, justifyContent: "center" }}>
                                <View>
                                    <Text style={[styles.subHeaderText, { fontWeight: "bold" }]}>
                                        {phoneFinal ? phoneFinal == "+91" ? DashboardReducer?.mainprofileResponse?.user_address?.contact_no : phoneFinal : phoneFinal}
                                    </Text>
                                </View>
                                <TouchableOpacity disabled={countdown == 0 ? false : true} onPress={() => {
                                    props.navigation.navigate("LoginMobileChange", { Newphone: { Newphone: phoneDetect == "+91" ? DashboardReducer?.mainprofileResponse?.user_address?.contact_no : phoneDetect || phoneDetect || props?.route?.params?.validPh || finalCode?.nationalNumber || props?.route?.params?.newPh || props?.route?.params?.mobileNo?.mobileNo || props?.route?.params?.validPh?.cellno, phonoCd: finalCode, PhoneCdO: props?.route?.params?.validPh?.phonecode || props?.route?.params?.mobileNo?.phoneCode || props?.route?.params?.Newphone?.phoneCode } });
