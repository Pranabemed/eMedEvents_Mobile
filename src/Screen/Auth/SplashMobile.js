...
const SplashMobile = (props) => {
    ...
    useEffect(() => {
        if (props?.route?.params?.newPh && !AuthReducer?.resendmobileotpResponse?.phone_otp) {
            setTimeout(() => {
                setShowtext("text");
            }, 2000);
        }
    }, [props?.route?.params?.newPh, AuthReducer?.resendmobileotpResponse?.phone_otp])
    useEffect(() => {
        if (showtext == "text") {
    ...
    console.log(DashboardReducer?.mainprofileResponse, "mainprofile---------", props?.route?.params, showtext)
    ...
    const phoneDetect = props?.route?.params?.Newphone?.allNo || props?.route?.params?.validPh?.validPh || props?.route?.params?.Newphone?.phoneCode || props?.route?.params?.Newphone?.phone || AuthReducer?.verifyResponse?.phone || props?.route?.params?.newPh || props?.route?.params?.mobileNo?.mobileNo || props?.route?.params?.Newphone?.phoneCode || AuthReducer?.verifyResponse?.phone || allotpcheckddd;
    const phoneFinal = phoneDetect?.startsWith('+')
        ? phoneDetect
        : `${props?.route?.params?.validPh?.phonecode || props?.route?.params?.mobileNo?.phoneCode || props?.route?.params?.Newphone?.allNo || ''} - ${phoneDetect}`;
    console.log(phoneFinal, "phonedetect=========", AuthReducer?.verifyResponse?.phone, props?.route?.params?.Newphone?.allNo)
    useEffect(() => {
