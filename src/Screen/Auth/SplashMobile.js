...
const SplashMobile = (props) => {
    ...
    // Tracks last handled status to prevent double-opening modal
...
const fetch = () => {
    ...
    // ─── Dispatch wallet/prime/profile when signupResponse arrives ────────────
    useEffect(() => {
    ...
    // ─── Dispatch wallet/prime/profile on screen focus ────────────────────────
    useEffect(() => {
        setTimeout(async () => {
...
const restore = async () => {
    ...
    // ─── Auto-trigger OTP ONCE when newPh param is present ───────────────────
    //
    // Uses a module-level variable (_splashMobileOTPSentForPhone) as the guard.
    ...
    //
    useEffect(() => {
        const phone = props?.route?.params?.newPh;
        if (!phone) return;

        // Synchronous check + set — no await, no race condition possible
        if (_splashMobileOTPSentForPhone === phone) return;
        _splashMobileOTPSentForPhone = phone; // locked before any async work
    ...
    // useEffect pattern (NOT render-body switch) — avoids mutation during render
    useEffect(() => {
                        ...
                        <View style={{ flexDirection: 'column', marginTop: normalize(15) }}>
                            ...
                            <View style={{ flexDirection: 'row', gap: 5, justifyContent: 'center' }}>
                                ...
                                <TouchableOpacity
                                    ...
                                    onPress={() => {
                                        props.navigation.navigate('SplashMobileChange', {
                                            Newphone: {
                                                Newphone: phoneDetect ||
                                                    props?.route?.params?.validPh ||
