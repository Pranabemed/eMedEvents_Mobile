/**
 * Change mail screen module. Renders a React Native screen or a screen-scoped support component. Exported members: status, ChangeMail, handleChangeEmail, styles.
 */
    ...
    if (status == '' || AuthReducer.status != status) {
        switch (AuthReducer.status) {
            ...
            case 'Auth/changeemailSuccess':
                status = AuthReducer.status;
                props.navigation.navigate("VerifyOTP", { NewEmail: { "email": email ? email: props?.route?.params?.wholeData?.email,phoneNo:props?.route?.params?.wholeData?.phone || props?.route?.params?.wholeData?.phoneNo, verifyotp: AuthReducer?.changeemailResponse?.email_otp,returnDat:props?.route?.params?.wholeData } });
                break;
        ...
        <>
            ...
            >
                <SafeAreaView style={{ flex: 1, backgroundColor: Colorpath.Pagebg }}>
                    ...
                    <View>
                        <Header
                            onPress={() => props.navigation.navigate("VerifyOTP",{NewEmail:props?.route?.params?.wholeData})}
                    ...
                    <View style={{ flexDirection: "row", justifyContent: "center", marginTop: normalize(20) }}>
                        <TouchableOpacity onPress={() => { props.navigation.navigate("VerifyOTP",{NewEmail:props?.route?.params?.wholeData || AuthReducer?.verifyResponse?.email}); }}>
