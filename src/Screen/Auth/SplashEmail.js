...
const VerifyOTPEmail = (props) => {
    ...
    console.log("enteredOTP === allotpcheck", AuthReducer?.resendemailotpResponse?.email_otp,AuthReducer);
    const verifyHandle = () => {
    ...
    useEffect(() => {
        if (props?.route?.params?.newMail && !AuthReducer?.resendemailotpResponse?.email_otp) {
            setTimeout(() => {
              setEmailtext("email");
          }, 2000);
        }
    }, [props?.route?.params?.newMail, AuthReducer?.resendemailotpResponse?.email_otp]);
    useEffect(() => {
        if (emailtext == "email") {
    ...
    const email = props?.route?.params?.NewEmail?.email || props?.route?.params?.NewEmail || props?.route?.params?.newMail || props?.route?.params?.mobileNo || props?.route?.params?.user?.emailid || AuthReducer?.verifyResponse?.email || allotpcheck;
    const phoneTake = props?.route?.params?.user?.phoneData || props?.route?.params?.mobileNo?.phone || props?.route?.params?.verifyemail?.verifyemail?.phone || props?.route?.params?.NewEmail?.phoneNo || props?.route?.params?.NewEmail?.phone || props?.route?.params?.NewEmail?.returnDat?.phone || props?.route?.params?.NewEmail?.returnDat?.phoneNo || AuthReducer?.verifyResponse?.phone;
    ...
    const high = props?.route?.params?.NewEmail?.email ? props?.route?.params?.NewEmail?.email : props?.route?.params?.NewEmail ? props?.route?.params?.NewEmail : props?.route?.params?.newMail;
                        ...
                        <View style={{ flexDirection: "column", marginTop: normalize(15) }}>
                            ...
                            {email?.length < 15 ? <View style={{ flexDirection: "row", gap: 5 }}>
                                ...
                                <TouchableOpacity disabled={countdown == 0 ? false : true} onPress={() => {
                                    props.navigation.navigate("ChangeMailSplash", {
                                        wholeData: high,
                                    });
                                    startNewTimer(0);
                                    clearAllOTPFields();
