...
const VerifyOTP = (props) => {
    ...
    const verifyHandle = () => {
        ...
        if (resendtrue && AuthReducer?.resendemailotpResponse?.email_otp) {
            ...
            serverOTP = AuthReducer?.signupResponse?.email_otp;
        }
...

                            {
                                email?.length < 15 ? <View style={{ flexDirection: "row", gap: 5 }}>
                                    ...
                                    < TouchableOpacity disabled={countdown == 0 ? false : true} onPress={() => {
                                        props.navigation.navigate("ChangeMail", { wholeData: props?.route?.params?.verifyemail?.verifyemail || props?.route?.params?.NewEmail });
                                        startNewTimer(0);
                                        clearAllOTPFields();
                                    ...
                                    < TouchableOpacity disabled={countdown == 0 ? false : true}
                                        ...
                                        onPress={() => {
                                            props.navigation.navigate("ChangeMail", {
                                                wholeData:
                                                    props?.route?.params?.verifyemail?.verifyemail ||
